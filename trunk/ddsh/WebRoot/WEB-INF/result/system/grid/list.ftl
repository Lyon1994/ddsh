    ...
    <head>
		...
		<script language="javascript">
			...
			function deletes()
			{
				...
				if(window.confirm("确定要删除这些格子吗？")){
					$.ajax({
					 	url: 'grid!del.zf?ids='+str+'&t='+new Date().getTime(),
					 	type: 'POST',
					 	dataType: 'json',
			...
			function load(param)
			{
				var b="<table class='maintab_content_table' width='100%'><thead><tr class='maintab_content_table_title'><th width='1%'><input type='checkbox' name='select' onclick='ck()'/></th><th>格子编号</th><th>名称</th><th>具体位置</th><th>负责人</th></tr></thead><tbody>";
				var a="</tbody></table>";
				$.ajax({
					 	url: 'grid!count.zf?type=1&t='+new Date().getTime(),
					 	type: 'POST',
					 	dataType: 'json',
					 	error: function(){alert('error');},
					 	success: function(json){
							//蓝色主题
							$('#list').jpage({dataBefore:b,dataAfter:a,dataStore: null,themeName:'blue',totalRecord:json[0],proxyUrl:'grid!result.zf?t='+new Date().getTime()+'&type=1',openCookies:false,
			...
			function exports()
			{
				window.open('grid!export.zf?type=1&t='+new Date().getTime());
			}
			$(document).ready(
				function(){
					//demo1带参数
