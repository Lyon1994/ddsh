		...
		<script language="javascript">
			...
			function backQuick()
			{
				...
				if(window.confirm("确定要批量退回这些记录吗？")){
					$.ajax({
					 	url: 'product!batchAdd.zf?ids='+str+'&t='+new Date().getTime(),
					 	type: 'POST',
					 	dataType: 'json',
					 	error: function(){alert('error');},
					 	success: function(json){
			...
			function back()
			{
				...
				var returnstr;
        		returnstr = window.showModalDialog('../html/product_back.jsp?id='+str.replace(',',''),'',"dialogHeight: 500px; dialogWidth: 750px;center: yes; help: no;resizable: no; status: no;");
				load('');
			...
			function load(param)
			{
				var b="<table class='maintab_content_table' width='100%'><thead><tr class='maintab_content_table_title'><th width='1%'><input type='checkbox' name='select' onclick='ck()'/></th><th>物品名称</th><th>单价</th><th>类型</th><th>状态</th><th>设计师</th><th>图片</th><th>数量</th><th>单价</th><th>总价</th><th>型号</th><th>规格</th><th>材料</th><th>产地</th><th>提交日期</th><th>备注</th></tr></thead><tbody>";
				var a="</tbody></table>";
				$.ajax({
					 	url: 'product!count.zf?type=0&t='+new Date().getTime(),
					 	type: 'POST',
					 	dataType: 'json',
					 	error: function(){alert('error');},
					 	success: function(json){
							//蓝色主题
							$('#list').jpage({dataBefore:b,dataAfter:a,dataStore: null,themeName:'blue',totalRecord:json[0],proxyUrl:'product!result.zf?t='+new Date().getTime()+'&type=0',openCookies:false,
			...
			function exports()
			{
				window.open('product!export.zf?type=0&t='+new Date().getTime());
			}
			$(document).ready(
				function(){
					//demo1带参数
