    ...
    <head>
		...
		<script language="javascript">
			...
			function enable()
			{
				...
				if(window.confirm("确定要启用这些记录吗？")){
					$.ajax({
					 	url: 'user2!enable.zf?ids='+str+'&t='+new Date().getTime(),
					 	type: 'POST',
					 	dataType: 'json',
			...
			function load(param)
			{
				...
				var a="</tbody></table>";
				$.ajax({
					 	url: 'user2!count.zf?t='+new Date().getTime(),
					 	type: 'POST',
					 	dataType: 'json',
					 	error: function(){alert('error');},
					 	success: function(json){
							//蓝色主题
							$('#list').jpage({dataBefore:b,dataAfter:a,dataStore: null,themeName:'blue',totalRecord:json[0],proxyUrl:'user2!result.zf',openCookies:false,
