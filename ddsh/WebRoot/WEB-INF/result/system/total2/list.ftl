...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
		...
		<script language="javascript">
			...
			function load(param)
			{
				var b="<table class='maintab_content_table' width='100%'><thead><tr class='maintab_content_table_title'><th>年份</th><th>销售额</th><th>退货额</th><th>净销售额</th></tr></thead><tbody>";
				var a="</tbody></table>";
				$.ajax({
					 	url: 'total2!count.zf?type=0&t='+new Date().getTime(),
					 	type: 'POST',
					 	dataType: 'json',
					 	error: function(){alert('error');},
					 	success: function(json){
							//蓝色主题
							$('#list').jpage({dataBefore:b,dataAfter:a,dataStore: null,themeName:'blue',totalRecord:json[0],proxyUrl:'total2!result.zf?t='+new Date().getTime()+'&type=0',openCookies:false,
			...
			function exports()
			{
				window.open('total2!export.zf?type=0&t='+new Date().getTime());
			}
			$(document).ready(
				function(){
					//demo1带参数
