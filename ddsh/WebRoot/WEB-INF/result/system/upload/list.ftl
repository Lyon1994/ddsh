		...
		<script language="javascript">
			...
			function deletes()
			{
				...
				if(window.confirm("确定要删除这些记录吗？")){
					$.ajax({
					 	url: 'upload!del.zf?ids='+str+'&t='+new Date().getTime(),
					 	type: 'POST',
					 	dataType: 'json',
			...
			function load(param)
			{
				var b="<table class='maintab_content_table' width='100%'><thead><tr class='maintab_content_table_title'><th width='1%'><input type='checkbox' name='select' onclick='ck()'/></th><th>条形码</th><th>物品名称</th><th>数量</th><th>操作人</th><th>日期</th></tr></thead><tbody>";
				var a="</tbody></table>";
				$.ajax({
					 	...
					 	success: function(json){
							//蓝色主题
							$('#list').jpage({dataBefore:b,dataAfter:a,dataStore: null,themeName:'blue',totalRecord:json[0],proxyUrl:'upload!result.zf?t='+new Date().getTime()+'&type=0',openCookies:false,
